...
@Composable
         ...
         * @usage
         * 1. Нажмите "Начать игру" для запуска игры.
         * 2. Нажмите "Настройки" для изменения параметров игры.
         * 3. Нажмите "Выход" для закрытия приложения.
         * @warning Нажатие кнопки "Выход" вызывает `killProcess`, что необратимо завершает приложение.
         * @traceability См. `StartScreenTest` для UI-тестов.
         * @priority High - Это точка входа в приложение.
         * @verification
         * - Проверка перехода на игровой экран при нажатии "Начать игру" (см. `testNavigationFromStartToGame` в `SnakeGameAppTest`).
         * - Проверка перехода на экран настроек (см. `testNavigationFromStartToSettings` в `SnakeGameAppTest`).
         */
fun StartScreen(
    onStartGame: () -> Unit,
    onSettings: () -> Unit,
